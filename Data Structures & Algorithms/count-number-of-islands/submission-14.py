class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        #DFS
        if not grid:
            return 0

        rows, cols = len(grid), len(grid[0])
        count = 0
        directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]
        visited = set()
        def dfs(row, col):
            if row < 0 or row >= rows or col < 0 or col >= cols or grid[row][col] == "0" or (row, col) in visited:
                return
            visited.add((row, col))
            for r, c in directions:
                nr, nc = row + r, col + c
                dfs(nr, nc)


        for row in range(rows):
            for col in range(cols):
                if grid[row][col] == "1" and (row, col) not in visited:
                    dfs(row, col)
                    count += 1
        return count







        # Questions:
        # 1. "Can the grid be empty?" (네가 한 거)
        # 2. "Are islands only connected horizontally/vertically, or diagonally too?" → 방향 정의가 달라짐
        # 3. "Can we modify the grid in place?" → visited set 쓸지 grid 자체 변경할지 결정


        # Solutions
        # DFS
        # if not grid:
        #     return 0
        # rows, cols = len(grid), len(grid[0])
        # visited = set()
        # directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]

        # def dfs(row, col):
        #     if row < 0 or row >= rows or col < 0 or col >= cols or grid[row][col] == "0" or (row, col) in visited:
        #         return
        #     visited.add((row, col))
        #     for row_dir, col_dir in directions:
        #         nr, nc = row_dir + row, col_dir + col
        #         dfs(nr, nc)

        # count = 0
        # for row in range(rows):
        #     for col in range(cols):
        #         if grid[row][col] == "1" and (row, col) not in visited:
        #             dfs(row, col)
        #             count += 1

        # return count

        # BFS
        # if not grid:
        #     return 0
        # rows, cols = len(grid), len(grid[0])
        # visited = set()
        # directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]

        # def bfs(row, col):
        #     q = deque()
        #     q.append((row, col))
        #     visited.add((row, col))
        #     while q:
        #         curr_row, curr_col = q.popleft()
        #         for rowDir, colDir in directions:
        #             nr, nc = curr_row + rowDir, curr_col + colDir
        #             if nr < 0 or nr >= rows or nc < 0 or nc >= cols or grid[nr][nc] == "0" or (nr, nc) in visited:
        #                 continue
        #             q.append((nr, nc))
        #             visited.add((nr, nc))
        
        # count = 0
        # for row in range(rows):
        #     for col in range(cols):
        #         if grid[row][col] == "1" and (row, col) not in visited:
        #             bfs(row, col)
        #             count += 1

        # return count












        # BFS
        # if not grid:
        #     return 0
        # rows, cols = len(grid), len(grid[0])
        # visited = set()
        # directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]

        # def bfs(row, col):
        #     q = deque()
        #     q.append((row, col))
        #     visited.add((row, col))

        #     while q:
        #         curr_row, curr_col = q.popleft()
        #         for rowDir, colDir in directions:
        #             nr, nc = curr_row + rowDir, curr_col + colDir
        #             if nr < 0 or nr >= rows or nc < 0 or nc >= cols or (nr, nc) in visited or grid[nr][nc] == "0":
        #                 continue
        #             q.append((nr, nc))
        #             visited.add((nr, nc))

        
        # count = 0
        # for row in range(rows):
        #     for col in range(cols):
        #         if grid[row][col] == "1" and (row, col) not in visited:
        #             bfs(row, col)
        #             count += 1
                

        # return count
        #-------------------------------------------------
# DFS
# 모든 셀 한 번씩 방문 → O(R × C)
# 각 셀에서 재귀 4방향 → O(4) = O(1)
# ─────────────────────────────
# Total: O(R × C)
# BFS
# 모든 셀 한 번씩 방문 → O(R × C)
# 각 셀에서 queue 4방향 → O(4) = O(1)
# ─────────────────────────────
# Total: O(R × C)